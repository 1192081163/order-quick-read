...
- GitHub repository name: `order-quick-read`
...
- GitHub repository rename from `email-order-reader` to `order-quick-read`.
- Update checker GitHub Release API URL.
...
The change does not rename the Python import package, command-line entry point, or IMAP parsing behavior.

## Data Flow

At runtime, branding is read from a small shared constant so the UI and tray use the same display name. Packaging scripts use explicit artifact names so GitHub Release downloads are predictable. The update checker points to the renamed GitHub repository and continues to pick the correct asset by platform suffix, so renamed assets still work as long as the Release contains `.exe` for Windows and `.dmg` for macOS.
...
- Update checker points to `1192081163/order-quick-read`.
